...
# @Time    : 2022/5/7 09:53
# @Author  : tc
# @File    : rider_check_username_test.py
# @Description :
...
class Testrider:
    ...
    def test_rider_check_username(self, caseinfo):
        # 获取ymal的请求数据request
    ...
    def test_rider_check_user_amount(self, caseinfo):
        # 获取yaml的请求数据request
        value = caseinfo['request']
        # 更新yaml文件里面的带$值
        yaml_data = YamlUtil.extractdata_render_params(value)
    ...
    # @pytest.mark.dependency(depends=['Testrider::test_rider_check_user_amount'])
    @pytest.mark.parametrize("caseinfo", YamlUtil().yaml_read("data_yaml", "rider_cost_user_amount.yaml"))
    def test_rider_cost_user_amount(self, caseinfo):
        # 获取yaml的请求数据request
        value = caseinfo['request']
        # 更新yaml文件里面的带$值
        yaml_data = YamlUtil.extractdata_render_params(value)
